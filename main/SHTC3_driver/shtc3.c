...
esp_err_t shtc3_read_measure(i2c_master_dev_handle_t dev_handle, shtc3_register_rw_t reg, float *data1, float *data2)
{
    esp_err_t ret;
    uint8_t b_read[6] = {0};
    uint8_t read_reg[2] = { reg >> 8, reg & 0xff };
    ...
    // shtc3_wake(dev_handle);
    // Read 4 bytes of data from the sensor
    ret = i2c_master_receive(dev_handle, b_read, 6, 200);
    ESP_RETURN_ON_ERROR(ret, TAG, "Failed to read data from SHTC3 sensor");
...
esp_err_t shtc3_start_measure(i2c_master_dev_handle_t dev_handle, shtc3_register_rw_t reg)
{
    esp_err_t ret;
    uint8_t read_reg[2] = { reg >> 8, reg & 0xff };
    ...
    ret = i2c_master_transmit(dev_handle, read_reg, 2, -1);
    ESP_RETURN_ON_ERROR(ret, TAG, "Failed to transmit data to SHTC3 sensor");
    // shtc3_sleep(dev_handle);
